...
def read_prompt(prompt: str) -> dict[str, str]:
    """
    Takes a prompt generated by prompt-forge and casts it to a dictionary.

    E.g. with `--prompt a, test --width 432 --height 768`, returns
    `{"prompt": "a, test", "width": "432", "height": "768"}`.
    """
    prompt_parts = prompt.split("--")
    # Remove empty values
    prompt_parts = filter(lambda part: len(part) > 0, prompt_parts)
    args = {}
    for prompt_part in prompt_parts:
        argument, value = prompt_part.split(maxsplit=1)
        args.update({argument: value})
    return args
    ...
    def run(
        ...
        if dry_run:
            ...
            if len(lines) <= 8:
                ...
                for line in lines[-3:]:
                    print(line)
            # Raise an error to stop generation (there might be a cleaner way of doing this)
